/**
 * `codepol graph fan-out` — rank files by `importeeCount`.
 *
 * Symmetric to `graph fan-in`. When a file path is supplied, only that
 * file's metrics are reported; otherwise the top-N files by importee
 * count are emitted, ties broken by URI.
 */
...
export type GraphFanOutOptions = {
  cwd: string;
  configPath: string;
  filePath?: string;
  top?: number;
...
function fanOutEntriesSelect(
  ...
  entries.sort((left, right) => {
    if (left.importeeCount !== right.importeeCount) {
      return right.importeeCount - left.importeeCount;
    }
...
export async function graphFanOutRun(options: GraphFanOutOptions): Promise<number> {
  ...
  try {
    ...
    const entries = fanOutEntriesSelect(graph.nodes, focusUri, top);
    ...
    if (format === 'json') {
      ...
      for (const entry of entries) {
        console.log(`${entry.importeeCount}\t${entry.workspaceRelativePath}`);
      }
    }
    return 0;
  } finally {
