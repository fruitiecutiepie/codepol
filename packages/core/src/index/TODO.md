...
- known gaps:
  ...
  - Python `TypeAwareTypeHierarchySource` binding does not exist yet — `@codepol/python-language-bridge` ships `pythonCallGraphSourceCreate` but not `pythonTypeHierarchySourceCreate`. Until it does, type-aware overlays for Python are unavailable even if the host wires pyright.
