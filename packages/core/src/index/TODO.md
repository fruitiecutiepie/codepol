...
- [x] Python binding — `@codepol/python-language-bridge` package supplies `pythonCallGraphSourceCreate({ transport, symbolLocate, symbolIdResolve })`. Same shape as the TS bridge against `pyright` / `pylance` over standard LSP `prepareCallHierarchy` + `incoming/outgoingCalls`. The bridge does NOT spawn the language server itself; the host owns the transport lifecycle. Python-specific `callKind` derivation: `abstract` / `protocol` / `overload` / `dynamic` / `virtual` ⇒ `'dynamic-dispatch'`; `callback` / `higher-order` ⇒ `'higher-order'`; everything else ⇒ `'direct'` (the conservative default).
- [x] Tests: `packages/core/src/index/typeAwareCallGraphSourceRegistry.spec.ts` (registry round-trip), `tests/workspace-service.call-graph-type-aware.spec.ts` (every row of the conflict-resolution table + source-rejection / `requireTypeAware` paths), `packages/typescript-language-bridge/src/typeScriptCallGraphSource.spec.ts` and `packages/python-language-bridge/src/pythonCallGraphSource.spec.ts` (contract tests against a fake transport), daemon round-trip extended in `packages/workspace-service/src/daemon.spec.ts`

### 6. Type Relations
**Status**: Implemented (TypeScript / TSX / Python; structural-shape matching is TS-only — see Python adapter TODO)
...
- [x] Tree-sitter queries — TypeScript / TSX `languages/typescript/queries/typeRelations.ts` (class extends, class implements, abstract class extends/implements, interface extends); Python `languages/python/queries/typeRelations.ts` (positional bare-identifier superclasses; Python doesn't distinguish extends from implements syntactically so all parents map to `relationKind: 'extends'`)
...
- [x] Cross-file `resolvedTargetId` resolution — Step 6 in `crossFileResolve` (`indexBuilder.ts`) follows import bindings to resolve `resolvedTargetId` to the actual exported symbol from the source module. Works with re-export chains and aliased imports. Language-agnostic by construction: Python `from .animals import Animal` followed by `class Dog(Animal):` resolves through the same pipeline as TypeScript `import { BaseService } ...` + `class DerivedService extends BaseService`. See `tests/index.type-relations.python.spec.ts > resolves cross-file extends through a from-import` for the Python regression sentinel.
...
- [x] Unit tests (5 IndexStore + 3 ProjectIndex) and integration tests (15 in `tests/index.type-relations.spec.ts` for TS, 8 in `tests/index.type-relations.python.spec.ts` for Python — covering single/multi-parent, no-parent, `Generic[T]` and `typing.Protocol` exclusion, `subTypesGet` round-trip, cross-file resolution, and determinism)
- [x] Phase 9.4 / 9.5 tests: `tests/index.member-shape-extraction.spec.ts` (TS extractor matrix), `tests/index.structural-shape-resolution.spec.ts` (cross-file shape comparison), `tests/workspace-service.type-hierarchy-structural.spec.ts` (workspace integration), `tests/workspace-service.type-hierarchy-type-aware.spec.ts` (type-aware merge + Python host-bridge wiring), `tests/lsp.server.spec.ts` (in-process host registration path), `packages/core/src/index/typeAwareTypeHierarchySourceRegistry.spec.ts` (registry round-trip), `packages/typescript-language-bridge/src/typeScriptTypeHierarchySource.spec.ts` + `packages/python-language-bridge/src/pythonTypeHierarchySource.spec.ts` (binding contract tests)
- known gaps:
  - structural-shape (`memberShape`) extraction is TypeScript-only. Python `queryTypeHierarchy({ includeStructural: true })` returns declared `extends` only; the `no-undeclared-implementer` architecture rule never fires on Python files. See `packages/core/src/index/TODO_ADAPTER_PY.md` for the Python-specific design notes (Python's structural-shape semantics need a separate decision because duck typing has no static `implements` declaration to verify against).
  - Python type-aware overlays now have a shipped bridge (`pythonTypeHierarchySourceCreate`) plus host registration seams in workspace-service, but the repo still does not bundle a concrete pyright / pylance transport provider module. Until a host supplies that transport, Python `queryTypeHierarchy` falls back to declared / structural edges by default.
