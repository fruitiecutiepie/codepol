...
User-facing surfaces landed for Phase 7:

- **Shared panel grammar.** Three additive helpers in [extension-vscode/src/panels/panelShared.ts](extension-vscode/src/panels/panelShared.ts) — `panelChipRowHtml` (uniform `<div class="panel-chip-row">` + `<button class="panel-chip">` with caller-provided `data-*` routing attrs and an optional `locked` flag), `panelTallyHeaderHtml` (per-tier counts line, zero-count entries collapse), `panelLegendHtml` (legend block with SVG swatches that share the same edge-class names as the canvas) — plus three Phase-7 helpers (`panelLensSwitcherHtml`, `panelModePillHtml`, `panelConfidenceBannerHtml`) and a `PANEL_SHARED_CSS` block embedded once by the shell. The call-graph panel ([extension-vscode/src/panels/callGraphRender.ts](extension-vscode/src/panels/callGraphRender.ts)) and type-hierarchy panel ([extension-vscode/src/panels/typeHierarchyRender.ts](extension-vscode/src/panels/typeHierarchyRender.ts)) now route every chip row, the tier tally, and the legend through these helpers; the file-graph chip strip in `graphControlsHtml` ([extension-vscode/src/panels/render.ts](extension-vscode/src/panels/render.ts)) keeps its existing `data-control-*` attribute scheme (the `controls.ts` reducer + `BASE_SCRIPT` dispatcher are already widely tested) and inherits the visual grammar through the shared CSS — the parity layer is the look, not the dispatch surface. Doc note added inline so future readers know why the file-graph markup is exempt.
- **Activated confidence + kind chip rows on the call-graph panel.** Phase 9.2's `Confidence` and `Kind` chips were placeholder-only (`pointer-events: none`) until this slice. Both rows now render through the shared `panelChipRowHtml` helper (no `cg-chip-row-inert` wrapper), the BASE_SCRIPT dispatcher posts new `callGraphConfidenceToggle` / `callGraphKindToggle` messages, and `CallGraphPanelViewModel.controls` carries the per-tier active state (chips default to all-active so a user opening the panel sees every edge; toggling a chip narrows the visible set; an empty filter set normalizes back to all-active so a user can't render an empty subgraph by accident). The Confidence row also exposes a `panel-chip-row-note` ("Populated when a TypeAwareCallGraphSource is wired") so users understand when type-aware edges will start to appear. The manager's chip-message handler is a no-op today — wiring the filter through the rebuilder closure is a follow-up; the protocol contract is in place so the day-of cost is one rebuilder change.
- **Per-tier counts header on the call-graph panel.** New `CallGraphConfidenceCounts = { structural: number; typeAware: number }` field on `CallGraphPanelViewModel` ([extension-vscode/src/callGraphViewModels.ts](extension-vscode/src/callGraphViewModels.ts)); counts are tallied as edges are filtered through the direction / depth view-model pass so the line matches what the user sees on the canvas, not the raw payload. The header tally renders below the existing `<p class="cg-summary">` line through `panelTallyHeaderHtml` — same shape as the type-hierarchy panel's `N declared · M shape-matched · K from language server` line.
- **Lens switcher.** Each architecture-graph panel now renders an optional `<nav class="panel-lens-switcher">` header strip when the controller supplies a `lensSwitcher` payload. File-focus panels (dependency graph, architecture links) advertise `Module Graph` and `Architecture Links` lenses; symbol-focus panels (call graph, type hierarchy) advertise `Call Graph` and `Type Hierarchy` lenses. Cross-type switching (file ↔ symbol) is intentionally out of scope for the MVP — the lens switcher only lists lenses compatible with the current focus type. Click → `BASE_SCRIPT` posts `{type: 'panelLensSet', lens, focus}` (focus is the JSON-encoded `PanelLensFocus`), which `CodepolPanelManager` validates through `panelLensValueParse` / `panelLensFocusParse` and forwards to the host's new `panelLensOpen` action. The host wires the action to `controller.showDependencyGraph` / `showArchitectureLinks` / `showCallGraph` / `showTypeHierarchy` so the manager never needs a circular reference to `CodepolCommandController`. Lens-switcher payloads ride through every existing rebuilder closure so chip / filter toggles never drop the header strip. Cross-type lens switching is captured as a follow-up below.
- **Signature-impact mode on the call-graph panel.** New `mode?: 'signature-impact'` field on `CallGraphPanelViewModel`. When set, the panel renders a locked `Signature impact — callers, unbounded` mode pill (`panelModePillHtml`) above the chip rows, the direction / depth chips render with `panel-chip-locked` + `aria-disabled` + a BASE_SCRIPT short-circuit so chip clicks never re-fire `queryCallGraph` while the mode is locked, and a structural-confidence banner (`panelConfidenceBannerHtml`, `Structural confidence — dynamic dispatch and higher-order calls are not tracked.`) renders above the SVG. The mode pill's `×` button posts `{type: 'panelModeClear', modeId: 'signature-impact'}` which the manager forwards to the host's `panelModeClear` action; the host re-opens the panel via `controller.showCallGraph` (without `mode`) using the already-resolved focus symbol. The `mode` field is a string union, not a boolean, so future Phase 8 metric lenses (`mode: 'instability'`) can ship without a new panel kind.
- **`codepol.extension.peekSignatureImpact` command.** New entry in [extension-vscode/package.json](extension-vscode/package.json) (`contributes.commands` + `editor/context` `navigation@13`, gated by `codepol.indexBackedCommandsEnabled`). Registered in [extension-vscode/src/extension.ts](extension-vscode/src/extension.ts) with constant `CODEPOL_EXTENSION_COMMAND_PEEK_SIGNATURE_IMPACT`. Handler `CodepolCommandController.peekSignatureImpact` ([extension-vscode/src/commands.ts](extension-vscode/src/commands.ts)) is a thin wrapper over `showCallGraph` with `mode: 'signature-impact'`; the cursor-path guard (`FUNCTION_KINDS = { 'function', 'method' }`) bails with a friendly error when the cursor lands on a non-callable kind so users never see an empty panel.
- **Tests landed (Phase 7 polish — 49 new cases across 3 files):**
  - [tests/extension-vscode.panel-lens-switcher.spec.ts](tests/extension-vscode.panel-lens-switcher.spec.ts) (new file, 23 cases) — pure-helper tests for `panelChipRowHtml` (DOM grammar, locked variant, empty short-circuit, extra-class hooks, optional note row), `panelTallyHeaderHtml` (zero-count omission, every-zero short-circuit, extra-class hook), `panelLegendHtml` (entry shape + swatch class round-trip), `panelLensSwitcherHtml` (button per available lens, current-lens highlight, JSON focus payload, single-lens short-circuit, file-typed focus shape), `panelModePillHtml` (configuration suffix, em-dash separator, clear button), `panelConfidenceBannerHtml` (message, optional inline action, empty short-circuit), `PANEL_SHARED_CSS` (every helper selector exported); manager-level routing tests for `panelLensSet` (symbol-focus → host action call, malformed payloads dropped) and `panelModeClear` (focus echoed back through the host action).
  - [tests/extension-vscode.call-graph-panel.spec.ts](tests/extension-vscode.call-graph-panel.spec.ts) (+5 new cases) — mode pill + locked chips + confidence banner appear when `mode: 'signature-impact'`; default-mode renders without any of those elements; per-tier tally + Confidence/Kind chip rows + legend appear in the default render; lens-switcher header renders when supplied. The pre-existing inert-chip assertion was flipped to reflect the new "active by default" semantics (the chips are interactive in Phase 7).
  - [tests/extension-vscode.commands.spec.ts](tests/extension-vscode.commands.spec.ts) (+4 new cases) — `showCallGraph` attaches a symbol-focused lens-switcher payload (`currentLens: 'callers'`, both lenses, focus echo); `peekSignatureImpact` happy path (locked direction `callers` + locked depth `unbounded` + `mode: 'signature-impact'` + `queryCallGraph` invoked without a `depth` field on the wire); `peekSignatureImpact` rejects an interface cursor with the FUNCTION_KINDS error; `peekSignatureImpact` shows the custom cursor-prompt error when the cursor is on no symbol.
  - All 1079 tests across 100 test files pass after the change; 38 extension-vscode test files / 347 cases pass with the `lensSwitcher` payload threaded through every show-call.

Deferred:

- **Cross-type lens switching (file ↔ symbol).** The lens switcher only renders compatible lenses for the current focus type. Switching from a file panel to a symbol panel needs a "pick a symbol from this file" hop the MVP doesn't ship. Tracked here so the surface stays honest about its scope.
- **Per-tier filter behavior on the call-graph panel.** Confidence + Kind chips render and dispatch messages, but the manager's chip handler is a no-op today: filter state isn't threaded through the call-graph rebuilder closure. Wiring it requires extending `CallGraphPanelRebuilder` to accept the filter set; the contract change is small but waits until a real `TypeAwareCallGraphSource` binding ships and produces edges users actually want to filter. The chip rendering and message dispatch are already in place so the day-of cost is one rebuilder signature update.
- **Lens-switcher coverage on the dependency-path / dead-modules panels.** Both panels render in the same shell and could surface a lens row, but their focus shapes are different (a `(from, to)` pair for dependency path, an entry-point list for dead modules). Adding lens switching would force a richer focus encoding; deferred until those panels grow another lens worth switching to.
...
- **Python structural-shape surface is Protocol-only.** The Phase 9.4 follow-up landed Python `memberShape` extraction for explicit `Protocol` contracts, so `queryTypeHierarchy({ includeStructural: true })`, the type-hierarchy panel's "M shape-matched" count, and the `no-undeclared-implementer` rule now work on `.py` files when the contract owner is a `Protocol` / `typing.Protocol` declaration. The remaining limitation is scope, not absence: the extractor does not infer arbitrary class-to-class duck typing, and it does not treat `self.name = ...` assignments inside method bodies as structural properties yet. See [packages/core/src/index/TODO_ADAPTER_PY.md](packages/core/src/index/TODO_ADAPTER_PY.md) for the current Python-specific notes.
- **Python type-aware host transport/provider (Phase 9.5 follow-up).** `@codepol/python-language-bridge` already ships `pythonTypeHierarchySourceCreate`, and the workspace host now has a first-party registration seam (`workspaceTypeAwareBridgeSourcesRegister(...)` plus `CODEPOL_TYPE_AWARE_BRIDGE_PROVIDER`). The remaining follow-up is shipping a concrete pyright / pylance transport provider for that seam. Until a host supplies it, Python type-hierarchy queries still fall back to declared / structural edges by default.
...
- **Python type-aware host transport/provider (Phase 9.5 follow-up).** The bridge factory already exists; the remaining work is a concrete pyright / pylance transport provider for the first-party host registration seam.
