...
- known gaps (carried as Q5 in this note's open questions):
  - **dynamic dispatch and higher-order calls are not tracked.** A method invoked through an interface variable, an event emitter, a function passed as a callback, or any other late-bound site does not appear as a callee. The call graph is structural — it sees only direct, name-resolved invocations
  - **calls that flow through re-exports are not resolved.** When `b.ts` re-exports `helper` from `a.ts` and `c.ts` calls `helper` through `b.ts`, the call edge stops at the re-export boundary because `ProjectIndex.callersGet` / `calleesGet` use file-local resolution. Workaround for callers who care: walk the export chain manually using `exportLocationsGet` / `importResolve` before issuing the call-graph query
  - **structural typing is not modeled in type hierarchy.** Only declared `extends` / `implements` relations are followed. A class that satisfies an interface by shape (no `implements` clause) does not appear as a subtype
  - **symbol-id discovery is out of scope for MVP.** The workspace contract takes a `symbolId` but the public surface has no general-purpose lookup-by-name endpoint. Editor surfaces today get the id by walking file-level structures (`querySemanticDefinition`, the workspace-symbol search) before issuing the graph query; a richer entry-point capability is folded into a follow-up under Phase 5's hover work
