...
| Dynamic `import()` — specifier extraction | Integration | `tests/index.cross-file-resolution.spec.ts` | Exists (`import('./module')` captured as `ImportsRelation` via reused `import.source`/`import.side_effect` captures; no `ImportBindingRelation` created — binding resolution is future work) |
...
| 13 | CommonJS require() + dynamic import() | Medium | Small (6 tests, query patterns only — adapter extraction already existed) | CommonJS `require()` is common in Node.js codebases and needed for mixed CJS/ESM projects. Dynamic `import()` specifier extraction enables module dependency awareness for lazy-loaded modules. | Done (5 require tests: whole-module, destructured, ESM interop, module graph inclusion, external package. 1 dynamic import test: specifier extraction as `ImportsRelation`. Tree-sitter query patterns added with `#eq?` predicate for require; dynamic import reuses `import.source`/`import.side_effect` captures. Adapter extraction for require was pre-existing in `adapterCore.ts`; dynamic import binding resolution deferred as future work.) |
...
- **Dynamic imports** (`import("module")`): **Partially implemented.** Tree-sitter query pattern added to TypeScript `IMPORTS_QUERY`, reusing the `import.source`/`import.side_effect` captures. This captures the module specifier as an `ImportsRelation` for dependency awareness. **Not yet implemented:** binding resolution for `const mod = await import("./module")` (would require linking the variable declaration to the dynamic import call expression and creating an `ImportBindingRelation`). Dynamic imports do not yet appear in the module graph because the graph is built from `ImportBindingRelation` entries, not `ImportsRelation`.
...
Remaining future work (documented, not yet tested):
- Dynamic `import()` binding resolution (`const mod = await import("./module")` as `ImportBindingRelation`)
- Dynamic `import()` module graph integration (requires resolving `ImportsRelation` specifiers or creating `ImportBindingRelation` entries)
