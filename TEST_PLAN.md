...
| `crossFileResolve` — type-only named exports (`export type { Foo }`) | Integration | `tests/index.cross-file-resolution.spec.ts` | Skipped (tree-sitter grammar uses different node structure for `export type { }` vs `export { }`; needs export query extension) |
| `crossFileResolve` — anonymous default class export | Integration | `tests/index.cross-file-resolution.spec.ts` | Skipped (no name node to capture; needs synthetic symbol creation in `exportsExtract`) |
| `crossFileResolve` — anonymous default function export | Integration | `tests/index.cross-file-resolution.spec.ts` | Skipped (same as anonymous class — no name node for `export.default_name` capture) |
...
- **Type-only named exports** (`export type { Foo }`): **Not working.** The tree-sitter grammar emits a different node structure for `export type { }` than `export { }`. The current export query's `export_clause` / `export_specifier` patterns do not match type-only export syntax. Test is skipped in `tests/index.cross-file-resolution.spec.ts`. Needs a new tree-sitter query pattern targeting the type-only export node.
- **Anonymous default exports** (`export default class { }`, `export default function() { }`): **Not working.** The current default export handling only captures named declarations via `export.default_name`. Anonymous class/function defaults produce no name node, so no symbol is created and no `ExportsRelation` is emitted. Two tests are skipped in `tests/index.cross-file-resolution.spec.ts`. Fix requires: (1) new tree-sitter query patterns for anonymous default class/function, (2) synthetic symbol creation in `exportsExtract` or `symbolsExtract`.
...
Remaining skipped tests (4 total across 2 files):
- `tests/e2e.cli.spec.ts`: `--watch` (complex async lifecycle — chokidar watcher with debounced re-runs)
- `tests/index.cross-file-resolution.spec.ts`: `export type { Foo }` type-only named exports (tree-sitter grammar uses different node structure; needs export query extension)
- `tests/index.cross-file-resolution.spec.ts`: `export default class {}` anonymous default class export (no name node to capture; needs synthetic symbol creation)
- `tests/index.cross-file-resolution.spec.ts`: `export default function() {}` anonymous default function export (same as anonymous class)
